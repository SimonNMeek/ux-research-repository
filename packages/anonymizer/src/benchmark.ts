...
function generateTestText(size: number): string {
  const template = `
...
async function runBenchmarks(): Promise<void> {
  ...
  for (const size of sizes) {
    try {
      ...
      console.log(
        `${size}\t\t${result.duration}\t\t${(result.throughput / 1024).toFixed(2)}\t\t${result.matches}\t${result.memoryUsage}`
      );
    } catch (error) {
  ...
  for (const size of sizes.slice(2)) { // Only test larger sizes for streaming
    for (const chunkSize of chunkSizes) {
      try {
        ...
        console.log(
          `${size}\t\t${chunkSize}\t\t${result.duration}\t\t${(result.throughput / 1024).toFixed(2)}\t\t${result.matches}\t${result.memoryUsage}`
        );
      } catch (error) {

...
export class RegexDetector extends BaseDetector {
  ...
  private validateMatch(type: string, value: string): boolean {
    switch (type) {
      case 'CARD':
  ...
  private validateNINumber(ni: string): boolean {
  ...
  private validateNHSNumber(nhs: string): boolean {
  ...
  private validateUKPostcode(postcode: string): boolean {

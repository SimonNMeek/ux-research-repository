...
5. **Option A (Recommended)**: Use the OpenAPI schema URL:
   - **Schema**: `https://ux-repo-web.vercel.app/.well-known/openapi.yaml`
   
   **Option B**: Paste the OpenAPI schema directly:
...
**⚠️ Important: Schema Loading Behavior**

ChatGPT loads plugin schemas (OpenAPI definitions) **only when a conversation starts**, not during an active conversation. This means:

- **New conversations** will see the latest schema with all available tools
- **Existing conversations** will continue using the schema that was loaded when they started
- If you update the OpenAPI schema (e.g., add new tools), existing conversations won't see them until you start a new conversation

**If a tool appears missing in an active conversation:**
1. Check that the OpenAPI schema is correctly deployed at `/.well-known/openapi.yaml`
2. Start a **new conversation** to load the updated schema
3. The tool should appear in the new conversation

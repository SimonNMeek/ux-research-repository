...
export async function POST(request: NextRequest) {
  try {
    ...
    return await withRlsContext({ userId: user.id }, async () => {
      ...
      const orgResult = await query<{ id: number; max_workspaces: number }>(
        `SELECT id, max_workspaces FROM organizations WHERE id = $1`,
        [organizationId]
      );
      ...
      const workspaceCountResult = await query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM workspaces WHERE organization_id = $1`,
        [organizationId]
      );
